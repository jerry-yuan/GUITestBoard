...
#define W25X_DMA_Channel_Rx DMA1_Channel4
#define W25X_DMA_Channel_Tx DMA1_Channel5

static uint8_t SectorBuffer[W25X_SECTOR_SIZE]={0};
...
void W25X_Initialize() {
    ...
    GPIO_InitStructure.GPIO_Pin =GPIO_Pin_12; //PA15 =1
...
void W25X_ReceiveBuffer(void* buffer,uint16_t length){
	...
	DMA_ClearFlag(DMA1_FLAG_GL4|DMA1_FLAG_GL5);
	...
	while(DMA_GetFlagStatus(DMA1_FLAG_TC4)==RESET);	//TODO: 改成信号量调度的
	// 关掉DMA
...
void W25X_Write_Buffer(uint32_t uiAddress,uint8_t* pBuffer,uint32_t uiLength) {
	...
	while(uiLength>0){
		...
		// 读出当前扇区现有的数据
		W25X_Read_Data(uiSectorAddress,SectorBuffer,W25X_SECTOR_SIZE);
		...
		for(uint16_t uiByteIndex=0;uiByteIndex<uiWriteLength;uiByteIndex++){
			...
			uiFlags |= ((~SectorBuffer[uiAddressOffset+uiByteIndex])&pBuffer[uiByteIndex])?1:0;
			// 将数据覆盖到缓冲区
			SectorBuffer[uiAddressOffset+uiByteIndex] = pBuffer[uiByteIndex];
		}
		if(uiFlags){
			...
			puiSectorBuffer     = SectorBuffer;
		}else{
			// 无需擦除 => 直接写入对应区域即可
			uiSectorWriteAddress = uiAddress;
			uiSectorWriteLength = uiWriteLength;
			puiSectorBuffer     = SectorBuffer+uiAddressOffset;
		}
		// 开始页编程
